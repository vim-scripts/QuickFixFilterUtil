...
" removes _darcs .hg and .svn files from the quickfix file list
...
" start region 'file <dotvim>/autoload/vl/lib/quickfix/filtermod.vim'
" "|fld   description : utility functions to filter / modify the quickfix result
" "|fld   keywords : quickfix 
" "|fld   initial author : Marc Weber marco-oweber@gmx.de
" "|fld   mantainer : author
" "|fld   started on : 2007 May 17 19:28:49
" "|fld   version: 0.0
...
" "|H1__  Documentation
" "|
" "|H2_   Enabling manual Filter:
" "|
" "|p    Put this into a new file plugin/quickfix_filter.vim
" "|code call vl#lib#quickfix#filtermod#EnableQuickFixScript()
" "|
" "|H2_   Enabling manual Filter:
" "|p     Put this into a new file ftplugin/qf.vim to remove .hg .svn .git and
" "|+     .darcs files automatically
" "|code  call vl#lib#quickfix#filtermod#DefaultProcessing()
" 
" 
" " registers an autocommand to automatically filter .svn _darcs and .hg entries
" function! vl#lib#quickfix#filtermod#EnableQuickFixScript()
"   augroup QuickFix
"     "au QuickFixCmdPost * call vl#lib#quickfix#filtermod#DefaultProcessing()
"     " filter lines
"     au BufNewfile,BufRead * if &buftype=='quickfix' | map <buffer> <m-f><m-l> :exec 'call vl#lib#quickfix#filtermod#FilterQFListByRegex('.string(input('qf: keep re: ')).', 0)'<cr>| endif
"     " drop lines
"     au BufNewfile,BufRead * if &buftype=='quickfix'  | map <buffer> <m-r><m-l> :exec 'call vl#lib#quickfix#filtermod#FilterQFListByRegex('.string(input('qf: drop re: ')).', 1)'<cr>| endif
"   augroup end
" endfunction
" 
...
" endfunction
" 
" " fix cygwin filenames and remove .svn and _darcs and .hg files from result
" " list (eg when grepping)
" function! vl#lib#quickfix#filtermod#DefaultProcessing()
"   if exists('b:processed')
"     return
"   endif
"   let b:processed = 1
"   " TODO: Use vl#lib#vimscript#closure#Call instead of different functions
"   call vl#lib#quickfix#filtermod#ProcessQuickFixResult('vl#lib#quickfix#filtermod#FilterVCHiddenFiles')
"   call vl#lib#quickfix#filtermod#ProcessQuickFixResult('vl#lib#quickfix#filtermod#FixCygwin')
" endfunction
" 
" function! vl#lib#quickfix#filtermod#ProcessQuickFixResult(functionName)
"   let qfl = getqflist()
"   let c = len(qfl)
"   " add filenames (this is the field I'm most likely interested in
"   call map(qfl, "( v:val['bufnr'] > 0 ? vl#lib#listdict#dict#SetReturn(v:val,'filename', bufname(v:val['bufnr'])) : v:val)")
"   exec 'let list = vl#lib#vimscript#closure#Call('.string(a:functionName).',[qfl])'
"   let c2 = len(list)
"   if c != c2
"     call vl#lib#ide#logging#Log(string(a:functionName).' has filtered '.(c2-c).' quickfix errors ')
"   endif
"   " avoid autocommand recursion
"   call setqflist(list)
" endfunction
" 
" " (consider using FilterQFListByRegex instead)
" function! vl#lib#quickfix#filtermod#FilterList(drop, regex, list)
"   " TODO (test this!)
"   if a:drop
"     let comp = '!~'
"   else
...
"   endif
"   let list = a:list
"   call filter(list, "(!exists(".string("v:val['filename']").") ) || v:val['filename']".comp.string(a:regex))
"   return list
" endfunction
" 
" function! vl#lib#quickfix#filtermod#FixCygwin(list)
"   " TODO (test this!)
"   let list = a:list
"   if !isdirectory('/cygdrive/c') && has('win16') || has('win32') || has('win64')
"     " no cygwin vim, process
"     call map(list, "vl#lib#listdict#dict#SetReturn(v:val,'filename', substitute(v:val['filename'],'/cygdrive/\\(.\\)/','\\1:/,'g'))")
"   endif
"   return list
" endfunction
" 
" function! vl#lib#quickfix#filtermod#FilterVCHiddenFiles(list)
"   let list = a:list
"   let c = len(list)
"   call filter(list, " (!exists(".string("v:val['filename']").") ) || v:val['filename'] !~ ".string('_darcs\|\.hg\|\.svn\|\.git'))
"   let removed = len(list) - c
"   if removed > 0 
"     call vl#lib#ide#logging#Log("vl#lib#quickfix#filtermod#FilterVCHiddenFiles: removed ".removed." files from qf list")
"   endif
"   return list
" endfunction
" end region 'file <dotvim>/autoload/vl/lib/quickfix/filtermod.vim'
